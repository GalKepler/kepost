...
for pkg, relative_path in zip(
    ["qsipost", "bids", "bids"],
    [
        "bids/config/qsiprep.json",
        "layout/config/bids.json",
        "layout/config/derivatives.json",
    ],
):
    _pybids_spec = loads(Path(_pkgres(pkg, relative_path)).read_text())
    BIDS_DERIV_ENTITIES += [e["name"] for e in _pybids_spec["entities"]]
    BIDS_DERIV_PATTERNS += _pybids_spec["default_path_patterns"]

BIDS_DERIV_ENTITIES = frozenset(BIDS_DERIV_ENTITIES)
BIDS_DERIV_PATTERNS = tuple(BIDS_DERIV_PATTERNS)
