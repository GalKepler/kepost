...
for pkg, relative_path in zip(
    ...
    BIDS_DERIV_ENTITIES_DICT += _pybids_spec["entities"]
    BIDS_DERIV_ENTITIES += [e["name"] for e in _pybids_spec["entities"]]
    BIDS_DERIV_PATTERNS += _pybids_spec["default_path_patterns"]

# BIDS_DERIV_ENTITIES = frozenset(BIDS_DERIV_ENTITIES)  # type: ignore[assignment]
# BIDS_DERIV_PATTERNS = tuple(BIDS_DERIV_PATTERNS)  # type: ignore[assignment]
...
class DerivativesDataSink(_DDSink):
    out_path_base = ""
    _file_patterns = tuple(BIDS_DERIV_PATTERNS)
...
class _BIDSDataGrabberOutputSpec(TraitedSpec):
    ...
    mni_to_native_transform = OutputMultiObject(
        desc="Transformation from standard to native space"
    )
    native_to_mni_transform = OutputMultiObject(
        desc="Transformation from native to standard space"
    )
    t1w_preproc = OutputMultiObject(desc="Preprocessed T1w")
    t1w_brain_mask = OutputMultiObject(desc="T1w brain mask")
    dwi_reference = OutputMultiObject(
        desc="Single-volume (B=0) preprocessed DWI reference"
    )
    dwi_nifti = OutputMultiObject(desc="Preprocessed DWI series")
    dwi_bval = OutputMultiObject(desc="DWI bvalues file")
    dwi_bvec = OutputMultiObject(desc="DWI bvector file")
    dwi_mask = OutputMultiObject(desc="Preprocessed DWI brain mask")
    dwi_grad = OutputMultiObject(desc="DWI gradients file in MRTrix format")
    t1w_to_dwi_transform = OutputMultiObject(
        desc="Transformation from T1w to DWI space"
    )
    dwi_to_t1w_transform = OutputMultiObject(
        desc="Transformation from DWI to T1w space"
    )
    gm_probabilistic_segmentation = OutputMultiObject(desc="GM probseg in T1w space")
    wm_probabilistic_segmentation = OutputMultiObject(desc="WM probseg in T1w space")
    csf_probabilistic_segmentation = OutputMultiObject(desc="CSF probseg in T1w space")
    eddy_qc = OutputMultiObject(desc="EDDY quad outputs")
